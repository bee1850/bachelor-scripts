...
install_tcp_dump() {
    ...
    if [ -z "$PROXY" ]; then
        log_audit_probe "[!] No proxy settings detected. Skipping proxy configuration for tcpdump installation."
        ssh -q "$TARGET_USER"@"$TARGET_IP" "kubectl exec -n $NS_FRONT $NGINX_POD -- /bin/sh -c 'apk add --no-cache tcpdump > /dev/null 2>&1'"
    else
        log_audit_probe "[+] Configuring proxy settings for tcpdump installation."
...
case "$LAYER_NAME" in
  baseline|layer_1)
    install_tcp_dump
    log_audit_probe "[+] Installed tcpdump."
